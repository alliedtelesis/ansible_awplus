        ...
        for each in want:
            ...
            for every in have:
                ...
                if (
                    ...
                    and each_want.get("secondary") != every_have.get("secondary")
                    or len(each_want.keys()) == len(every_have.keys())
                ):
                    diff = True
                    break
                elif (
                    each_want.get("dhcp_client") != every_have.get("dhcp_client")
                    and each_want.get("dhcp_client") is not None
                ):
                    diff = True
                    break
                elif (
                    each_want.get("dhcp_hostname") != every_have.get("dhcp_hostname")
                    and each_want.get("dhcp_hostname") is not None
                ):
                    diff = True
                    break
                elif each_want.get("address") != every_have.get("address") and len(
                    each_want.keys()
                ) == len(every_have.keys()):
                    diff = True
                    break
                    ...
                    if ipv4:
                        ipv4 = (
                            ...
                            if self.verify_diff_again(
                                dict(want_dict).get("ipv4"), dict(have_dict).get("ipv4")
                            )
                            else ()
                        )
                else:
                    ...
                    for each in ipv4:
                        ...
                        if ipv4_dict.get("address") != "dhcp":
                            ...
                            if ipv4_dict.get(
                                ...
                                cmd = "ip address dhcp client-id vlan{0} hostname {1}".format(
                                    ipv4_dict.get("dhcp_client"),
                                    ipv4_dict.get("dhcp_hostname"),
                                )
                            elif ipv4_dict.get("dhcp_client") and not ipv4_dict.get(
                                "dhcp_hostname"
                            ):
                                cmd = "ip address dhcp client-id vlan{0}".format(
                                    ipv4_dict.get("dhcp_client")
                                )
                            elif not ipv4_dict.get("dhcp_client") and ipv4_dict.get(
                                "dhcp_hostname"
                            ):
