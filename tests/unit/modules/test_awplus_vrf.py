...
from ansible_collections.alliedtelesis.awplus.plugins.modules import awplus_vrf
...
class TestAwplusVrfModule(TestAwplusModule):
    ...
    def test_awplus_vrfs_local_override_state(self):
        ...
        set_module_args(dict(vrfs=vrfs, description="test string"))
        commands = ["no ip vrf test_1", "ip vrf run", "description test string"]
        self.execute_module(changed=True, commands=commands, sort=False)
    ...
    def test_awplus_vrf_route_import(self):
        set_module_args(
            dict(name="test_1", rd="3:100", route_import=["3:100", "4:100"])
        )
        commands = [
            "ip vrf test_1",
            "rd 3:100",
