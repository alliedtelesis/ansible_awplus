...
from ansible_collections.alliedtelesis.awplus.plugins.modules import awplus_linkagg
...
class TestAwplusLinkaggModule(TestAwplusModule):
    ...
    def load_fixtures(self, commands=None, transport="cli"):
        self.get_config.return_value = load_fixture("awplus_linkagg_config.cfg")
    ...
    def test_awplus_linkagg_remove_member(self):
        ...
        result = self.execute_module(changed=True)
        cmds = ["interface port1.0.3", "no channel-group"]
        self.assertEqual(cmds, result["commands"])
