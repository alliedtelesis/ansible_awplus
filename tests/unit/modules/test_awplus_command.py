...
from ansible_collections.alliedtelesis.awplus.plugins.modules import awplus_command
...
class TestAwplusCommandModule(TestAwplusModule):
    ...
    def tearDown(self):
        super(TestAwplusCommandModule, self).tearDown()
    ...
    def test_awplus_command_simple(self):
        set_module_args(dict(commands=["show version"]))
        result = self.execute_module()
        self.assertEqual(len(result["stdout"]), 1)
        self.assertTrue(result["stdout"][0].startswith(""))

    def test_awplus_command_multiple(self):
        set_module_args(dict(commands=["show version", "show version"]))
        result = self.execute_module()
        self.assertEqual(len(result["stdout"]), 2)
        self.assertTrue(result["stdout"][0].startswith(""))

    def test_awplus_command_wait_for(self):
        wait_for = 'result[0] contains "AlliedWare Plus"'
        set_module_args(dict(commands=["show version"], wait_for=wait_for))
        self.execute_module()

    def test_awplus_command_wait_for_fails(self):
        wait_for = 'result[0] contains "test string"'
        set_module_args(dict(commands=["show version"], wait_for=wait_for))
        self.execute_module(failed=True)
        self.assertEqual(self.run_commands.call_count, 10)

    def test_awplus_command_retries(self):
        wait_for = 'result[0] contains "test string"'
    ...
    def test_awplus_command_configure_not_warning(self):
        commands = ["configure terminal"]
