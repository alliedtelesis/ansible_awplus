...
class TestAwplusOpenFlowModule(TestAwplusModule):
    ...
    def test_awplus_openflow_add_existing_controller(self):
        ...
        self.assertEqual(
            "Controller already exists, please use a different address/ssl port",
            result["msg"],
        )
    ...
    def test_awplus_openflow_remove_nonexisitng_controller(self):
        set_module_args(
            dict(
                controllers=[
                    dict(name="oc2", protocol="ssl", address="192.56.8.3", ssl_port=8)
                ],
                state="absent",
            )
        )
    ...
    def test_awplus_openflow_invalid_port_name(self):
        set_module_args(dict(ports=[dict(name="vlan1", openflow=True)]))
